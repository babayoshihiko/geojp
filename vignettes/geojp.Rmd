...
```
```{r, echo = F, eval = T}
sfCensus <- geojp::read_census_tract(code_pref = 4, code_muni = 215, data_dir = "~/Documents")
head(sfCensus)
```
...
```
```{r, echo = F, eval = T}
sfCensus2 <- geojp::read_census_odcity("京都市", data_dir = "~/Documents")
head(sfCensus2)
```
...
```
```{r, echo = F, eval = T}
sfYouto <- geojp::read_landnuminfo_landuse(code_pref = 26, 100, year = 2011, data_dir = "~")
```
